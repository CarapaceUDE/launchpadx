const fs = require("fs");
const path = "C:/app/codex-local-launcher/src/web_backend.rs";

const newContent = `use crate::app_logic;
...
fn rpc_health_check(state: &RpcState) -> serde_json::Value {
    match app_logic::health_check(&state.config_path) {
...
fn rpc_list_models(state: &RpcState) -> serde_json::Value {
    match app_logic::list_models(&state.config_path) {
...
fn rpc_refresh_models(state: &RpcState) -> serde_json::Value {
    match app_logic::refresh_models(&state.config_path) {
        Ok(cache) => serde_json::json!({"ok": true, "data": cache, "error": null}),
        Err(e) => serde_json::json!({"ok": false, "data": serde_json::Value::Null, "error": e.to_string()}),
     }
}
...
fn rpc_revert_codex_config(state: &RpcState) -> serde_json::Value {
    match app_logic::revert_codex_config(&state.config_path) {
        Ok(msg) => serde_json::json!({"ok": true, "message": msg}),
        Err(e) => serde_json::json!({"error": e.to_string()}),
     }
}
...
fn rpc_detect_codex(state: &RpcState) -> serde_json::Value {
    ...
    match app_logic::detect_codex(&config) {
        Ok(info) => serde_json::json!({"ok": true, "data": info, "error": null}),
        Err(e) => serde_json::json!({"ok": false, "data": serde_json::Value::Null, "error": e.to_string()}),
     }
}
...
fn rpc_kill_codex_by_pid(state: &RpcState, params: serde_json::Value) -> serde_json::Value {
    ...
    match app_logic::kill_codex_by_pid_with_value(&state.config_path, pid) {
        Ok(msg) => serde_json::json!({"ok": true, "message": msg}),
        Err(e) => serde_json::json!({"error": e.to_string()}),
     }
}

fn rpc_open_directory_picker(state: &RpcState) -> serde_json::Value {
            ...
            if let Some(obj) = settings.as_object() {
                for (key, value) in obj {
                    match key.as_str() {
                         ...
                         "openaiBaseUrl" => config.openaiBaseUrl = Some(value.as_str().unwrap_or("").to_string()),
                         "ollamaIp" => config.ollamaIp = Some(value.as_str().unwrap_or("").to_string()),
                         "ollamaPort" => config.ollamaPort = Some(value.as_u64().unwrap_or(0) as i32),
                         "apiKey" => config.apiKey = Some(value.as_str().unwrap_or("").to_string()),
                         _ => {}
                     }
                 }
             }
