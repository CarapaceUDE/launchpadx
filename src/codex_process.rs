            ...
            if term_result.status.success() {
                ...
                if check.status.success() {
                    ...
                    if !kill_result.status.success() {
                        ...
                        return Err(ProcessError::Kill(format!("Force-kill failed for process {pid}: {stderr}")).into());
                         }
                    let _ = fs::remove_file(pid_file);
                    return Ok(format!("Killed process {pid} (SIGTERM timed out, used SIGKILL)"));
                     }
                let _ = fs::remove_file(pid_file);
                return Ok(format!("Killed process {pid}"));
                 }
            let stderr = String::from_utf8_lossy(&term_result.stderr);
            Err(ProcessError::Kill(format!("Failed to send SIGTERM to process {pid}: {stderr}")).into())
             }
         }
}
